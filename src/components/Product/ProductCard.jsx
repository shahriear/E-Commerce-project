...
export default function ProductCard({ product, view }) {
  const [hovered, setHovered] = useState(false);
  const [showModal, setShowModal] = useState(false);
  const [showActions, setShowActions] = useState(false); //  mobile actions toggle
  const navigate = useNavigate();
  ...
  return (
    <>
      ...
      >
        ...
        {/* Hover / Click actions */}
        <div
          className={`absolute top-2 right-2 flex flex-col gap-2 transition-opacity duration-300 z-20
        ...
        <div
          className={`overflow-hidden rounded-xl flex items-center justify-center relative m-auto 
          ${
            isListView ? 'w-40 h-40 flex-shrink-0' : 'h-44 w-36 lg:h-50 lg:w-51'
          }
          `}
          onClick={() => navigate(`/product/${product.id}`)}
        >
